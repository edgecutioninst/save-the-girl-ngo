...
function drawScaledCenteredText(
  page: any,
  text: string,
  font: PDFFont,
  defaultSize: number,
...
export async function POST(req: Request) {
  try {
    ...
    if (!fileName) {
      return NextResponse.json(
        { error: `Unsupported certificate type: ${typeKey}` },
        { status: 400 },
      );
    }
    ...
    const formatDt = (dt: Date | null) =>
      dt ? new Date(dt).toLocaleDateString("en-IN") : "N/A";
    ...
    switch (typeKey) {
      case "VOLUNTEER":
        ...
        page.drawText(volRoleText, {
          x: center - volRoleWidth / 2,
          y: 532,
          size: 36,
          font: boldFont,
          color: textColor,
        });
        page.drawText(formatDt(startDate), {
          x: 554,
          y: 427,
          size: 32,
          font: boldFont,
        ...
        page.drawText(formatDt(endDate), {
          x: 1205,
          y: 427,
          size: 32,
          font: boldFont,
      ...
      case "INTERN":
        ...
        page.drawText(internRoleText, {
          x: center - internRoleWidth / 2,
          y: 565,
          size: 36,
          font: boldFont,
          color: textColor,
      ...
      case "VISITOR":
        ...
        const visItems = submission.itemsDonated as
        ...
        if (visItems && visItems.length > 0) {
          visDonationStr = options?.includeQuantity
            ? visItems
                .map((i) => `${i.item.toUpperCase()} x${i.quantity}`)
                .join(", ")
        ...
        const visWidthAtMaxSize = boldFont.widthOfTextAtSize(
          visDonationStr,
          visMaxItemFontSize,
        );
        ...
        if (visWidthAtMaxSize <= visDonMaxWidth) {
          ...
          const visDonationLines = wrapText(
            visDonationStr,
            visDonMaxWidth,
            boldFont,
            visWrapItemFontSize,
          ...
          let currentY =
            visBaselineY + (visDonationLines.length - 1) * visLineHeight;

          visDonationLines.forEach((line) => {
            const lineWidth = boldFont.widthOfTextAtSize(
              line,
              visWrapItemFontSize,
            );
            page.drawText(line, {
      ...
      case "DONOR":
        ...
        const donorItems = submission.itemsDonated as
          | { item: string; quantity: number }[]
          | null;
        ...
        const donorDonLineCenterItems =
          donorDonLineStartX + donorDonMaxWidth / 2;
        const baselineY = 630;

        const maxItemFontSize = 38;
        const wrapItemFontSize = 28;

        const widthAtMaxSize = boldFont.widthOfTextAtSize(
          donorDonationStr,
          maxItemFontSize,
        );
        ...
        if (widthAtMaxSize <= donorDonMaxWidth) {
          ...
          const donorDonationLines = wrapText(
            donorDonationStr,
            donorDonMaxWidth,
            boldFont,
            wrapItemFontSize,
          ...
          const lineHeight = 35;
          let currentY =
            baselineY + (donorDonationLines.length - 1) * lineHeight;
      ...
      case "HOST":
        const hostDate =
          submission.visitDate || submission.startDate || submission.createdAt;

        page.drawText(formatDt(hostDate), {
          x: 158,
          y: 1050,
          size: 34,
          font: boldFont,
        ...
        const hostFacilityStr = (
          submission.facilityLocation || "Our Center"
        ).toUpperCase();
        const hostFacWidth = boldFont.widthOfTextAtSize(hostFacilityStr, 38);
        const hostFacLineCenter = 791 + (rightMarginX - 791) / 2;
        page.drawText(hostFacilityStr, {
          x: hostFacLineCenter - hostFacWidth / 2,
          y: 760,
          size: 38,
          font: boldFont,
          color: textColor,
        ...
        const hostItems = submission.itemsDonated as
          | { item: string; quantity: number }[]
          | null;
        ...
        const hostWidthAtMaxSize = boldFont.widthOfTextAtSize(
          hostDonationStr,
          hostMaxItemFontSize,
        );
        ...
        if (hostWidthAtMaxSize <= hostDonMaxWidth) {
          ...
          const hostDonationLines = wrapText(
            hostDonationStr,
            hostDonMaxWidth,
            boldFont,
            hostWrapItemFontSize,
          );
          const hostLineHeight = 35;
          let currentY =
            hostBaselineY + (hostDonationLines.length - 1) * hostLineHeight;
    ...
    if (options?.sendEmail && options?.targetEmail) {
      try {
        ...
        let customSubject =
          "Thank you from Save The Girl! 🌸 Your Certificate Inside";

        switch (typeKey) {
          case "DONOR":
            emailDisplayActivity = "Donation";
            customSubject =
              "Thank you from Save The Girl! 🌸 Your Certificate of Donation Inside";
            break;
          case "HOST":
            emailDisplayActivity = "Hosting Partner";
            customSubject =
              "Thank you from Save The Girl! 🌸 Your Certificate of Hosting Partner Inside";
            break;
          case "VISITOR":
            emailDisplayActivity = "Visit";
            customSubject =
              "Thank you from Save The Girl! 🌸 Your Certificate of Visit Inside";
            break;
          case "INTERN":
            emailDisplayActivity = "Internship";
            customSubject =
              "Thank you from Save The Girl! 🌸 Your Certificate of Internship Inside";
            break;
          case "VOLUNTEER":
            emailDisplayActivity = "Volunteering";
            customSubject =
              "Thank you from Save The Girl! 🌸 Your Certificate of Volunteering Inside";
            break;
        }
        ...
        const mailOptions: any = {
          ...
          replyTo: "edgecutionist@gmail.com",
          bcc: "edgecutionist@gmail.com",
        ...
        console.log(
          `Successfully emailed certificate to ${options.targetEmail}`,
        );
        ...
        try {
          ...
          console.log(
            `Successfully updated lastSentEmail for submission ${id}`,
          );
        } catch (dbError) {
